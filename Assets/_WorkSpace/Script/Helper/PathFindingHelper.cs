...
namespace AutoChess
{
    ...
    public class PathFindingHelper : Singleton<PathFindingHelper>
    {
...
#pragma warning restore CS0649
            ...
            PositionModel positionModel) where T : LandModel
        {
            ...
            aroundPosition.Except (EmptyPosition);
            return aroundPosition;
        }

        
        /// <summary>
        /// 점진적으로 주변 위치를 포함하여 리턴.
        /// </summary>
        public IEnumerable<PositionModel> GetAroundPositionModelByGradual<T> (Dictionary<int, List<T>> landDict,
            PositionModel positionModel, int count) where T : LandModel
        {
            ...
            void GetAround ()
            {
                var around = resultPositions.SelectMany (position => GetAroundPositionModel (landDict, position));
                var pickedAround = around.Distinct ().Except (resultPositions).Except (EmptyPosition).RandomSources (count).ToList ();
                resultPositions.AddRange (pickedAround);
                count -= pickedAround.Count ();
            }
        }
